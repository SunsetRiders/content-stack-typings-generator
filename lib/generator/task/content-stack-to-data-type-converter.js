...
function createGroupType(field, library, parentType) {
    var typeName = parentType + "_" + string_utils_1.toPascalCase(field.uid);
    var fields = field.schema.map(function (x) {
        return new object_model_1.DataTypeField(x.uid, extractTypeName(x, library, typeName), x.mandatory, x.multiple);
    });
...
function addToLibrary(library, contentType) {
    ...
    var fields = contentType.schema.map(function (x) { return new object_model_1.DataTypeField(x.uid, extractTypeName(x, library, typeName), x.mandatory, x.multiple); });
